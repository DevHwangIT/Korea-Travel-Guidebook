...
def cleanup() -> None:
    from lib.paths import MEALS_DIR
...
def main() -> int:
    ...
    print("1) create dish")
    dish_notes, _ = content.create_dish(
        "meals",
        DISH_A,
        {
    ...
    dish_cover_path(DISH_A, "meals").write_bytes(jpeg)

    print("2) create shop + 2 menu images")
    shop_notes, _ = content.create_shop(
        "meals",
        DISH_A,
        SHOP_A,
        {
    ...
    assert menus[0].path.name == "menu-1.jpg"
    assert menus[1].path.name == "menu-2.jpg"
    ...
    assert "media/cover.jpg" in html
    ...
    assert f"./{SHOP_B}/" in idx
    assert f"./{SHOP_A}/" not in idx
    assert f"./{SHOP_A}.html" not in idx

    print("4) rename dish (moves shop folder with it)")
