...
2. 식사·디저트 **생성/이름변경/삭제** 시 먹거리 추천 카탈로그도 함께 갱신됩니다 (가게·명소·문장 저장은 카탈로그를 다시 돌리지 않음)
3. 수동 일괄 갱신: CMS **사이트 새로고침** (`/version`) 또는 `tool/update-version.bat` — 카탈로그 + `SITE_ASSET_VERSION`/`?v=`
4. 공개 화면에서 **Ctrl+F5** (강력 새로고침) — 또는 `/viewer`를 다시 엽니다
5. 반드시 **http://127.0.0.1:8765/...** 로 확인하세요 (`file://`로 연 탭은 예전 파일을 붙잡을 수 있음)
...
공개 사이트·CMS 저장 모두 **ko / en / ja / zh / zh-Hant / vi / th / ru** 를 지원합니다.
CMS는 한국어로 쓰고 저장 시 나머지 언어를 자동 채웁니다.
번역 API가 없거나 엔진이 해당 언어를 미지원하면 **영문(없으면 한국어) 복사**로 키를 채웁니다.
(유용한 한국어 문장 데이터는 공개 UI가 EN·JA 의미만 보여주는 구조입니다.)
