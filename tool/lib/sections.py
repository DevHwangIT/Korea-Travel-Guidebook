...
SECTIONS: list[SectionDef] = [
    ...
    SectionDef(
        "prepTips",
        "준비 & 여행팁",
        "준비·여행팁 허브 문구 (before-trip 확장 허브·팁 목록 안내)",
        "prepTips",
        group_by_prefix=True,
        preview_path="pages/before-trip/",
        board_group="준비 & 여행팁",
    ...
    SectionDef(
        "travelCourses",
        "여행 코스 추천",
        "일정 길이별 여행 코스 추천 (당일·1박2일·2박3일·3박4일)",
        "travelCourses",
        group_by_prefix=True,
        preview_path="pages/travel-courses/",
        board_group="여행 코스 추천",
    ),
    SectionDef(
        "emergency",
        "긴급 연락",
    ...
    SectionDef(
        "homeMenus",
        "홈 메뉴 라벨",
        "메인 화면 메뉴 카드 제목 (준비·여행팁·여행 코스 포함)",
        "home",
        key_filter=[
            "menuApps",
            "menuBeforeTrip",
